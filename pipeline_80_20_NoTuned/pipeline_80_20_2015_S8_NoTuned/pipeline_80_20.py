...
def run_once(file, window_start, window_end, lag_setting, zero_policy,
    ...
    # Stage-1 fit on TRAINING set; transform TRAINING & TEST set
    ...
    # Stage-2 tuning per target (nested K-fold on TRAINING set)
    best_stage2 = {}
    for t in TARGETS:
        other = [c for c in TARGETS if c != t][0]
        if tuned:
    ...
    # final Stage-2 fit on TRAINING set and predict TEST
    preds = {}
    for t in TARGETS:
        other = [c for c in TARGETS if c != t][0]
        Xt_tr = pd.concat([X_imp_tr, Yc_tr[[other]]], axis=1)
