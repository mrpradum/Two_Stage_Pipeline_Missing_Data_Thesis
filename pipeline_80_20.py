...
CLI:
  python pipeline_80_20.py --cmd run \
      ...
      --lag both_lags --zero-policy standard --window 2015-07-01:2015-10-31 \
...
def run_once(file, window_start, window_end, lag_setting, zero_policy,
    ...
    # ---- Stage-1 tuning (nested K-fold on TRAIN) ----
    if tuned:
    ...
    # ---- Stage-1 fit on TRAIN; transform TRAIN & TEST ----
    ...
    # ---- Stage-2 tuning per target (nested K-fold on TRAIN) ----
    best_stage2 = {}
    for t in TARGETS:
        other = [c for c in TARGETS if c != t][0]
        if tuned:
    ...
    # ---- Final Stage-2 fit on TRAIN and predict TEST ----
    preds = {}
    for t in TARGETS:
        other = [c for c in TARGETS if c != t][0]
        Xt_tr = pd.concat([X_imp_tr, Yc_tr[[other]]], axis=1)
...
def main():
    ...
    if args.cmd == "run":
        ...
        # all combos in your methodology (3 lags × 2 windows × 2 zero policies)
